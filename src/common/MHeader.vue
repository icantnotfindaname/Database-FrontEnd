    ...
    <div>
      <header class="w">
        <div class="w-box">
          <div class="nav-logo">
            <h1>
              <router-link to="/" title="商城官网">SCUT商城</router-link>
            </h1>
          </div>
          <div class="right-box">
            <div class="nav-list">
                  ...
                  <div class="nav-user-list">
                    <ul>
                      ...
                      <li class="nav-user-avatar">
                        <div>
                          <span class="avatar" :style="{backgroundImage:'url(http://qkwb83dxb.hn-bkt.clouddn.com/Head.png)'}"></span>
                        </div>
					  ...
					  <li>
					    <router-link to="/user/commodity">商品管理</router-link>
					  </li>
                      <li>
                  ...
                  <div class="nav-user-list">
                    <div class="full">
                          ...
                          <li class="clearfix" v-for="(goods,index) in cartList" :key="index">
                            <div class="cart-item">
                              <div class="cart-item-inner">
                                <a @click="openProduct(goods.productId)">
                                ...
                                <div class="del-btn del" @click="deleteCart(goods.productId)">删除</div>
                              </div>
                            </div>
                          </li>
                        </ul>
                      ...
                      <div class="nav-cart-total">
                        ...
                        <h6  v-if="totalNum">
                          <el-button type="danger" @click="toCheckout">去结算</el-button>
                        </h6>
                      </div>
                    </div>
...
export default {
  ...
  async mounted() {
    if (this.login) {
      ...
      this.$http.get(`/server/api/cartList?userId=${getStore("id")}`)
      .then((res) => {
  ...
  methods: {
    ...mapMutations(["SHOWCART", "INITBUYCART", "DELCART", "INITUSRINFO"]),
    deleteCart(productId) {
      ...
      if (this.login) {
        ...
        // });
          ...
          this.$http.get(`/server/api/delCart?userId=${getStore("id")}&productId=${productId}`)
            .then(function (res) {
    ...
    openProduct(productId) {
      window.open(
        "//" + window.location.host + "/goodsDetail?productId=" + productId
      );
    },
    toCheckout() {
      // 未登录跳转登录页面，登录用户才可购买
...
.w-box {
  ...
  h1 {
    ...
    > a {
      ...
      background: url("http://qkwb83dxb.hn-bkt.clouddn.com/S.png") no-repeat 50%;
      background-size: cover;
      display: block;
      @include wh(45px, 50px);
      text-indent: -9999px;
