...
<script>
	...
	export default {
		...
		methods: {
			...
			getOrderList() {
				this.$http.get(
						`/server/admin/orders?size=${this.pageSize}&page=${this.currentPage}`
					)
					.then((res) => {
			...
			orderDetail(orderId) {
				window.open(window.location.origin + '/user/orderDetail?orderId=' + orderId)
			},
			...
			writeComment(orderId) {
				this.$prompt('请输入评论', '提示', {
					...
					// TODO 上传评论
					this.$http.get(
							`/server/user/orderList/comment?orderId=${orderId}&comment=${value}`
						)
						.then((res) => {
							console.log(res);
							if (res.data.success) {
