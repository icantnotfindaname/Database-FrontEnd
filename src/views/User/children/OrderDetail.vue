			...
			<div slot="content">
				<div v-loading="loading" element-loading-text="加载中..." style="min-height: 10vw;" v-if="orderList.length">
					...
					<div class="gray-sub-title cart-title">
						<div class="first">
							<div>
								<span class="order-id">卖家信息</span>
							</div>
						</div>
					</div>
					<div style="height: 100px;padding: 20px 30px;">
						<p class="address">姓名：{{ sellerName }}</p>
					</div>
					<div class="gray-sub-title cart-title">
						<div class="first">
							<div>
					...
					<div style="height: 125px;padding: 20px 30px;">
						<p class="address">姓名：{{ userName }}</p>
						<p class="address">详细地址：{{ address }}</p>
					</div>
					<div class="gray-sub-title cart-title" v-if="orderStatus == 1">
						<div class="first">
							<div>
								<span class="order-id">我的评论</span>
							</div>
						</div>
					</div>
					<div style="padding: 20px 30px;">
						<p class="commment">{{comment}}</p>
					</div>
				</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getOrder() {
				this.$http.get(
						`/server/user/orderDetail?orderId=${this.orderId}`
					)
					.then((res) => {
						console.log(res);
						if (res.data.success) {
							this.orderList = res.data.orderDetail.orderList
							this.createTime = res.data.orderDetail.createDate
							if(res.data.orderDetail.orderStatus === '1'){ 
								this.finishTime = res.data.orderDetail.finishTime
								// TODO
							    this.comment = res.data.orderDetail.comment
							}
							this.orderStatus = res.data.orderDetail.orderStatus
							this.userName = res.data.orderDetail.userName
							this.address = res.data.orderDetail.address
							this.sellerName = res.data.orderDetail.sellerName
							this.orderTotal = res.data.orderDetail.orderTotal
							this.loading = false
						} else {
							console.log("getOrder fails");
						}
					})
					.catch((error)=> {
						console.log(error);
